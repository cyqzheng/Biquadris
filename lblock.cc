...
LBlock::LBlock(std::vector<Cell *> positions, Grid * theGrid, int level): Block(positions, theGrid, level){
    curOrientation = 0;
}
...
void LBlock::rotateCw(){
    ...
    if(curOrientation==0){
        ...
        newpos.push_back(theGrid->getCell(position[0]->getRow()+0, position[0]->getCol()-1));
        ...
        newpos.push_back(theGrid->getCell(position[0]->getRow()+2, position[0]->getCol()-0));
        ...
        newpos.push_back(theGrid->getCell(position[0]->getRow()-1, position[0]->getCol()+2));
